...
- 2026-05-22 — Cron outage: the `*/15` Cloudflare cron stopped firing after the 08:15 UTC tick; SLO B2 (`cron_silent_30m`) fired at 08:50. Both `mlb_cron_runs` and Cloudflare's own Cron Events log confirmed zero invocations after 08:15:48 UTC. Investigation cleared the repo: `wrangler.jsonc`'s `triggers.crons` is unchanged since #76 and the cron-injection scripts since #165, and the production deploy log shows `wrangler deploy` correctly injecting the `scheduled()` handler and registering both triggers (`*/15 * * * *`, `0 13 * * *`). The triggers also show in the dashboard with valid next-run times — yet Cloudflare invoked none of them. A correct redeploy (09:29 UTC), a version rollback, and a dashboard delete/re-add of the triggers all failed to revive it, pointing to a Cloudflare-side cron scheduler fault rather than application code. Recovery: manual `/admin` break-glass runs (`Run daily scheduler now`, `Force run main cron`) kept recap email flowing while the platform issue was escalated. Followup: add an external cron (GitHub Actions → `/api/cron`) so a Cloudflare scheduler outage can no longer silence sends.

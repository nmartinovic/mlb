...
- 2026-05-17 — Cron silent failure (`supabaseUrl is required` on every `*/15` tick) caused by missing runtime Worker secrets for `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`; the values were set only in the Cloudflare Build store (where Next.js inlines them into HTTP bundles) so HTTP traffic stayed healthy and masked the outage. SLO B1 caught it ~26h in. Recovered by adding both keys to the runtime Worker secrets store (no deploy needed — Cloudflare hot-swaps). Full postmortem and followups (#165–#168) in [#164](https://github.com/nmartinovic/mlb/issues/164).

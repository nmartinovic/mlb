...
export async function resendLatestRecap({
  ...
  fetchNextGameImpl = fetchNextGame,
}) {
  let gamePk = null;
  let teamId = null;
  let gameDate = null;
  ...
  let nextGame = null;
  try {
    const next = await fetchNextGameImpl(teamId, gameDate);
    if (next) {
    ...
    console.warn(
      `resend-recap: next-game fetch failed for team ${teamId} (continuing without):`,
      err.message
    );
  }

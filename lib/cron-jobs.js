...
// Subject line for a recap email (#27). A single game keeps the original
// `{Team} Highlights — {Date}` form so one-game behavior is unchanged. A batch
// of two or more reads `Your {Team A} & {Team B} highlights — {Date}`, listing
// each distinct team (in encounter order) so a multi-team subscriber can still
// tell what's inside from the inbox. The date is the most recent game's date.
function buildRecapSubject(renderGames) {
  if (renderGames.length === 1) {
    const rg = renderGames[0];
    return `${rg.teamName} Highlights — ${formatDisplayDate(rg.game.gameDate)}`;
  }
  const names = [];
  for (const rg of renderGames) {
    const name = rg.team?.shortName || rg.teamName;
  ...
  const latestDate = renderGames
    .map((rg) => rg.game.gameDate)
    .sort()
    .at(-1);
  return `Your ${teamList} highlights — ${formatDisplayDate(latestDate)}`;
}
...
export async function runMainCron({
  ...
  try {
    ...
    // Group new games by recipient so a subscriber following multiple teams
    // whose games finished in the same window receives ONE batched email
    // rather than one per game (#27). Each game-team pair maps to every
    // subscriber of that team; a user subscribed to both teams in a single
    // game would otherwise see it twice, so each game_pk is added at most once
    // per user.
    ...
    for (const game of newGames) {
      ...
      for (const userId of subscriberUserIds) {
        if (!gamesByUser.has(userId)) gamesByUser.set(userId, []);
        const userGames = gamesByUser.get(userId);
        if (!userGames.some((g) => g.gamePk === game.gamePk)) {
          userGames.push(game);
        }
      }
    }
    ...
    for (const [userId, userGames] of gamesByUser) {
      ...
      if (!email) {
        for (const g of userGames) {
          skipped.push({ gamePk: g.gamePk, userId, reason: "no_email" });
        }
        continue;
      }
      ...
      if (pausedUserIds.has(userId)) {
        for (const g of userGames) {
          skipped.push({ gamePk: g.gamePk, userId, reason: "notifications_paused" });
        }
        continue;
      }

      // Drop games this user already received — dedup stays per game_pk, so
      // re-running the cron never re-sends a game already in
      // mlb_sent_notifications, batched or not.
      const freshGames = [];
      for (const g of userGames) {
        if (alreadySent.has(`${userId}:${g.gamePk}`)) {
          skipped.push({ gamePk: g.gamePk, userId, reason: "already_notified" });
        } else {
          freshGames.push(g);
        }
      }
      ...
      const renderGames = freshGames.map((g) => {
        const team = TEAMS_BY_ID[g.teamId];
        const standings = standingsByDate.get(g.gameDate);
        const standing = standings ? extractTeamStanding(standings, g.teamId) : null;
        const rawSeries = extractSeriesContext(g.scheduleGame, g.teamId);
      ...
      const subject = buildRecapSubject(renderGames);
      const html =
        renderGames.length === 1
          ? buildEmailHtml(
              renderGames[0].team,
              renderGames[0].game.highlightUrl,
              userId,
      ...
      if (dryRun) {
        ...
        // (reads, cache upsert, render) already ran for real. The report
        // keeps one entry per game so the operator sees every highlight,
        // even though those games ship in a single batched email.
        console.log(
          `DRY RUN: would send to ${email} — "${subject}" (${renderGames.length} game${
            renderGames.length === 1 ? "" : "s"
          })`
        );
        for (const rg of renderGames) {
          dryRunReport.push({
      ...
      try {
        ...
        // One row per game so dedup stays per game_pk; a single insert call
        // keeps the subrequest budget flat regardless of batch size.
        await supabase
          .from("mlb_sent_notifications")
          .insert(freshGames.map((g) => ({ user_id: userId, game_pk: g.gamePk })));

        emailsSent++;
      } catch (err) {
        const errMsg = `Failed to email ${email} for game${
          freshGames.length === 1 ? "" : "s"
        } ${freshGames.map((g) => g.gamePk).join(", ")}: ${err.message}`;
        console.error(errMsg);
        errors.push(errMsg);
      }
    }

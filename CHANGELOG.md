...
### Changed
- Branded the Supabase magic-link email and fixed a silent sign-in failure (closes #55, PRs #121 + follow-up). Three pieces shipped together because they're inseparable: the template, the sender, and the auth flow. **Template:** new `supabase/email-templates/magic-link.html` (paste-into-dashboard source of truth, not loaded at runtime) mirrors `lib/email-template.js` — 520px single-column card, 6px ballpark-green accent, CTA button on `{{ .ConfirmationURL }}`, footer wordmark and non-affiliation disclaimer matching the cron recap. Subject is `Your Ninth Inning Email login link`; subject and first body line both name the product so a recipient knows the source without clicking. **Sender:** already done in #97 (custom SMTP via Brevo, `Ninth Inning Email <highlights@ninthinning.email>`, SPF/DKIM/DMARC passing, no "via" suffix in Gmail) — #55 just verified it. **Auth flow:** `/api/login` now uses `createClient` from `@/lib/supabase-server` (the `@supabase/ssr` server client) instead of the raw `@supabase/supabase-js` client. The raw client defaulted to the **implicit OAuth flow**, which returns tokens in the URL hash (`#access_token=...`) where our server-side `/auth/callback` route can't read them; the SSR client defaults to **PKCE**, which redirects with `?code=...` that the existing `exchangeCodeForSession` call in `app/auth/callback/route.js` handles correctly. The mix was a silent bug — verify succeeded and the user landed on the redirect URL, but no session cookie was ever set and they were bounced back to `/login`. First test of the new template in production reproduced this; the fix went out as a follow-up commit on the same branch. Also hardened `emailRedirectTo` to prefer `process.env.SITE_URL` over the request origin, so a misrouted request (e.g. hitting the worker on `*.workers.dev`) can't ship a bad redirect URL — particularly relevant because Supabase silently falls back to **Site URL** when `emailRedirectTo` isn't on the **Auth → URL Configuration → Redirect URLs** allowlist (which is the second half of why the original test landed on `/login`). Required allowlist entries are documented in the new "Magic-link email template and auth flow" section of `CLAUDE.md`. New test in `app/api/login/route.test.js` locks in `SITE_URL` precedence over request origin
- Main cron now falls open when `mlb_cron_schedule` is empty during MLB regular season (closes #109, PR #117). The schedule table was a hidden hard dependency between two endpoints — `/api/cron` (every 15 min) cannot do its job without `/api/cron/schedule` (daily 9am ET) having run, and an empty table early-returned identically to "no game in window," so a single scheduler outage dropped up to 24h of emails (the proximate cause of the 2026-05-02 incident, postmortem #103, action D1). The early-return path now distinguishes "table populated, just nothing due now" (the common offseason / overnight case — keep the cheap `skipped_no_wake` heartbeat) from "table entirely empty" via a second unwindowed `.select('game_pk').limit(1)` against the same table. If empty **and** ET month ∈ [4..10] (a simple season heuristic via `Intl.DateTimeFormat({ timeZone: 'America/New_York' })`, since MLB exposes no clean season-active flag), the main cron falls through to the full check path and writes a normal `success` / `partial` / `no_new_highlights` row instead of the heartbeat. Outside the season the early-return stays — an empty table is the expected steady state. The early-return is now a pure efficiency optimization rather than a correctness gate. Trade-off accepted in the issue: ~96 extra full-run cron ticks per day during a scheduler outage in exchange for never silently dropping a day of emails. New tests cover all three branches (empty + in-season → fall-through, empty + offseason → heartbeat, populated + no wake → heartbeat); existing schedule-read-error fall-open and `EMAILS_PAUSED` short-circuit still hold. Updated the "Cron architecture" section of `CLAUDE.md` with the new branch and the revised "Daily scheduler fails" failure-mode entry. With #109 shipped, the postmortem #103 minimum-bar defenses (heartbeat → dashboard banner → out-of-band alarm → deploy bootstrap → in-season fall-open) are all in place; only E1/E2 (`/admin` "run now" buttons + `CRON_SECRET` recovery doc) remain from the suggested order
